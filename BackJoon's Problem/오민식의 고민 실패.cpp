...
int main(void)
{
     ...
     //   cout<<" 이동횟수 : "<<j<<endl;
        ...
        //    cout<<"도시번호 : "<<i<<endl;
            //i번도시에 서있다. 교통수단 j번 이용해서.
                 ...
                 //          cout<<"갱신 : "<<DP[k][j-1]<<" - "<<Root[k][i]<<" + "<<city[i]<<" = "<<DP[i][j]<<endl;
    ...
    //양의 싸이클 검사..................양의 싸이클....양의 싸이클이 있는지 검사하자 Root[i][j]가 i에서
            //출발하여 j로 도착하는 값어치 일때.....
    //

            //Floyd로 양의 사이클 검사.
            for(int i=0;i<N;i++)//N-1번 간선이 N-1개니까 반복.
                for(int j=0;j<N;j++)
                {
                    if(Root[i][j]!=INF)
            ...
            for(int k=0;k<N;k++)
                for(int i=0;i<N;i++)
                {
                    ...
                    for(int j=0;j<N;j++) {
                       ...
                       //     cout<<k<<" 를 거쳐서 "<<i<<" 에서 "<<j<<"로 가는 것"<<endl;
                            if(D[i][j]==INF)
                                D[i][j]=D[i][k]+D[k][j];
