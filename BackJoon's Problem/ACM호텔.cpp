...
//고객 설문조사에 따르면 손님들은 호텔 정문으로부터 걸어서 가장 짧은 
//거리에 있는 방을 선호한다고 한다.여러분은 지우를 도와 줄 프로그램을 
//작성하고자 한다.즉 설문조사 결과 대로 호텔 정문으로부터 걷는 거리가 
...
//호텔 정문은 일층 엘리베이터 바로 앞에 있는데, 정문에서 엘리베이터까지의 
//거리는 무시한다.또 모든 인접한 두 방 사이의 거리는 같은 거리(거리 1)라고 
//가정하고 호텔의 정면 쪽에만 방이 있다고 가정한다.
//
//방 번호는 YXX 나 YYXX 형태인데 여기서 Y 나 YY 는 층 수를 나타내고 
//XX 는 엘리베이터에서부터 세었을 때의 번호를 나타낸다.즉, 그림 1 
//에서 빗금으로 표시한 방은 305 호가 된다.
//
//손님은 엘리베이터를 타고 이동하는 거리는 신경 쓰지 않는다.다만 
//걷는 거리가 같을 때에는 아래층의 방을 더 선호한다.예를 들면 102 호 
//방보다는 301 호 방을 더 선호하는데, 102 호는 거리 2 만큼 걸어야 
//하지만 301 호는 거리 1 만큼만 걸으면 되기 때문이다.같은 이유로 102 
//호보다 2101 호를 더 선호한다.
//
